...
NekoType 是一款运行在 Android 上的系统级消息处理工具，核心目标只有一个：**让用户以最小的操作成本，把聊天输入框里的文本按照自定义规则改写后一键发送**。你正常打字，点击屏幕边缘常驻的悬浮按钮，消息会自动完成「文本替换 → 前缀 → 后缀 → 随机尾缀 → 自动发送」的完整流程。例如添加一条"前缀"规则值为 `回复：`、一条"后缀"规则值为 `（已读）`，输入 `好的`，发出的就是 `回复：好的（已读）`；添加一条"随机后缀"规则，则每条消息还会按触发概率追加不同的随机尾缀，让内容不再千篇一律。
...
| 规则类型 | 配置项 | 示例 |
|---|---|---|
| **前缀** | 内容 | 内容 `回复：` → 输入 `好的` 发出 `回复：好的` |
| **后缀** | 内容 | 内容 `（已读）` → 输入 `好的` 发出 `好的（已读）` |
